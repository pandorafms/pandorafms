...
# Enterprise console
cd $CODEHOME_ENT/pandora/branches/3.2/pandora_console/DEBIAN && bash ./make_deb_package.sh && mv ../*.deb $RPMHOME/DEB || exit 1

# Enterprise server
cd $CODEHOME_ENT/pandora/branches/3.2/pandora_server/PandoraFMS-Enterprise/DEBIAN && bash ./make_deb_package.sh && mv ../*.deb $RPMHOME/DEB || exit 1
