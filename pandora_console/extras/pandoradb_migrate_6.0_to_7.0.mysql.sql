...
INSERT INTO tnetwork_profile (id_np, name, description) SELECT * FROM (SELECT NULL id_np, @template_name name, @template_description description) AS tmp WHERE NOT EXISTS (SELECT id_np FROM tnetwork_profile WHERE name = @template_name);
