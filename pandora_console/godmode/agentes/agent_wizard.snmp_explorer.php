...
if (empty($plugin)) {
    ui_print_info_message(['message' => __('The SNMP remote plugin doesnt seem to be installed').'. '.__('It is necessary to use some features').'.<br><br>'.__('Please, install the SNMP remote plugin (The name of the plugin must be snmp_remote.pl)'), 'no_close' => true]);
}
...
if ($create_modules) {
    ...
    if (agents_get_name($id_agent) == false) {
        ...
        $devices_prefix_descriptions = [
            'diskIONRead'    => __('The number of bytes read from this device since boot'),
            'diskIONWritten' => __('The number of bytes written to this device since boot'),
            'diskIONReads'   => __('The number of read accesses from this device since boot'),
            'diskIONWrites'  => __('The number of write accesses from this device since boot'),
        ];
        ...
        foreach ($snmpdata as $snmpdata_oid) {
            ...
            $module_values['descripcion'] = $snmp_translation['description'];
            $module_values['id_tipo_modulo'] = modules_get_type_id('remote_snmp');
            ...
            switch ($snmpdata_oid) {
                ...
                case 'memoryUse':
                            ...
                            case '_field3_':
                                ...
                                if ($snmp_version == '3') {
                                    ...
                                    switch ($snmp3_security_level) {
                                        case 'authNoPriv':
                                            $macros[$k]['value'] .= ' -u '.$snmp3_auth_user.' -A '.$snmp3_auth_pass.' -l '.$snmp3_security_level.' -a '.$snmp3_auth_method;
                                        break;

                                        case 'noAuthNoPriv':
                                            $macros[$k]['value'] .= ' -u '.$snmp3_auth_user.' -l '.$snmp3_security_level;
                                        break;

                                        default:
                                            $macros[$k]['value'] .= ' -u '.$snmp3_auth_user.' -A '.$snmp3_auth_pass.' -l '.$snmp3_security_level.' -a '.$snmp3_auth_method.' -x '.$snmp3_privacy_method.' -X '.$snmp3_privacy_pass;
                                        break;
                                    }
                                }
                            break;
                    ...
                    unset($module_values['snmp_community']);
                    // snmp_community
                    unset($module_values['ip_target']);
                    // ip_target
                    unset($module_values['tcp_send']);
                    // snmp_version
                break;
            ...
            $result = modules_create_agent_module(
                $id_agent,
                io_safe_input($snmp_translation['description']),
                $module_values
            );
...
if (!$fail) {
    ...
    // Devices arrow
    $table->data[2][1] = '<div class="wizard_mode_form wizard_mode_devices wizard_mode_devices_arrow clickable">'.html_print_image('images/darrowright.png', true, ['title' => __('Add to modules list')]).'</div>';
    // Processes arrow
    ...
    // Temperatures arrow
    $table->data[2][1] .= '<div class="wizard_mode_form wizard_mode_temperatures wizard_mode_temperatures_arrow clickable">'.html_print_image('images/darrowright.png', true, ['title' => __('Add to modules list')]).'</div>';
    // SNMP data arrow
    if ($arrow) {
        $table->data[2][1] .= '<div class="wizard_mode_form wizard_mode_snmpdata wizard_mode_snmpdata_arrow clickable">'.html_print_image('images/darrowright.png', true, ['title' => __('Add to modules list')]).'</div>';
    }
