...
// Layers
$layer_ids = get_parameter("layer_ids", array());
$layers = get_parameter("layers", array());
$layer_list = array();

foreach ($layer_ids as $layer_id) {
	if (empty($layers[$layer_id]) || empty($layers[$layer_id]["name"])) continue;
	...
	case 'save_new':
		...
		if (empty($invalidFields) && get_parameter('map_connection_list') != "") {
			$idMap = gis_save_map($map_name, $map_initial_longitude, $map_initial_latitude,
				...
				$map_default_longitude, $map_default_latitude, $map_default_altitude,
				$map_group_id, $map_connection_list, $layer_list);
			$mapCreatedOk = true;
...
switch ($action) {
	...
	case 'update_saved':
		...
		$layer_list = !empty($mapData['layers']) ? $mapData['layers'] : array();
		break;
}
