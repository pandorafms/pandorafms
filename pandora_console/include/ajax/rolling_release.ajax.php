...
if (is_ajax()) {
    ...
    if ($updare_rr) {
        ...
        if ($dangerous_query) {
            ...
            $message = '<div>';
            $message .= "<div class='w25p float-left'><img class='pdd_t_20px pdd_l_20px' src='images/icono_error_mr.png'></div>";
            $message .= "<div class='w75p float-left'><h3><strong class='font_13pt verdana'>ERROR</strong></h3>";
            $message .= "<p class='font_12pt verdana'>".__('The sql file contains a dangerous query').'</p></div>';
            $message .= '</div>';
            ...
            if (file_exists($dir) && is_dir($dir)) {
                if (is_readable($dir)) {
                    if (($number > $config['MR'] + 1) || ($number == $config['MR'])) {
                        ...
                        if ($result) {
                            ...
                            $message .= "<div class='w25p float-left'><img class='pdd_t_20px pdd_l_20px' src='images/icono_error_mr.png'></div>";
                            $message .= "<div class='w75p float-left'><h3><strong class='font_13pt verdana'>ERROR</strong></h3>";
                            $message .= "<p class='font_12pt verdana'>".__('An error occurred while updating the database schema to the minor release ').$number.'</p></div>';
                            $message .= '</div>';
                    ...
                    $message .= "<div class='w25p float-left'><img class='pdd_t_20px pdd_l_20px' src='images/icono_error_mr.png'></div>";
                    $message .= "<div class='w75p float-left'><h3><strong class='font_13pt verdana'>ERROR</strong></h3>";
                    $message .= "<p class='font_12pt verdana'>".__('The directory ').$dir.__(' should have read permissions in order to update the database schema').'</p></div>';
                    $message .= '</div>';
                ...
                $message .= "<div class='w25p float-left'><img class='pdd_t_20px pdd_l_20px' src='images/icono_error_mr.png'></div>";
                $message .= "<div class='w75p float-left'><h3><strong class='font_13pt verdana'>ERROR</strong></h3>";
                $message .= "<p class='font_12pt verdana'>".__('The directory ').$dir.__(' does not exist').'</p></div>';
                $message .= '</div>';
