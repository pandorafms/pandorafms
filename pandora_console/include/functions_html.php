        ...
        foreach ($table->data as $keyrow => $row) {
            ...
            foreach ($row as $key => $item) {
                ...
                if ($class === 'datos5' && $key === 1) {
                    $output .= '<td id="'.$tableid.'-'.$keyrow.'-'.$key.'" style="'.$cellstyle[$keyrow][$key].$style[$key].$valign[$key].$align[$key].$size[$key].$wrap[$key].' width: 30%" '.$colspan[$keyrow][$key].' '.$rowspan[$keyrow][$key].' class="'.$class.' '.$cellclass[$keyrow][$key].'">'.$item.'</td>'."\n";
                } else {
