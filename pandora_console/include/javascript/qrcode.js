...
(function(root, factory) {
   ...
   */
  var getFormattedPhoto = function(
    photoType,
    url,
    mediaType,
    base64,
  ...
  function getSocialUrls(type, user) {
    switch (type.toLowerCase()) {
      ...
      case "linkedin":
        return "https://linkedin.com/" + user;
      case "twitter":
  ...
  QrCode.prototype.createVCard = function(vCard) {
    ...
    if (vCard.workPhone) {
      ...
      vCard.workPhone.forEach(function(number) {
        if (majorVersion >= 4) {
          formattedVCardString +=
            'TEL;VALUE=uri;TYPE="voice,work":tel:' + e(vCard.workPhone) + nl();
        } else {
          formattedVCardString +=
            "TEL;TYPE=WORK,VOICE:" + e(vCard.workPhone) + nl();
        }
      });
    }
      ...
      for (var key in vCard.socialUrls) {
        if (vCard.socialUrls.hasOwnProperty(key) && vCard.socialUrls[key]) {
          formattedVCardString +=
            ...
            ":" +
            getSocialUrls(key, vCard.socialUrls[key]) +
            nl();
        }
      }
    }
