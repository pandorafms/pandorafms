/* x_core.js compiled from X 4.0 with XC 0.27b.
...
if(window.opera){
	...
	if(i!=-1){
		...
		document.all && xUA.indexOf('msie')!=-1){
...
function xBackground(e,c,i){
	...
	if (e.style) {
		...
		if(xStr(i))
			e.style.backgroundImage=(i!='')? 'url('+i+')' : null;if(!xOp6Dn) bg=e.style.backgroundColor;
		else
...
function xHasPoint(e,x,y,t,r,b,l){
	if (!xNum(t)){t=r=b=l=0;}else if (!xNum(r)){r=b=l=t;}else if (!xNum(b)){l=r; b=t;}var eX = xPageX(e), eY = xPageY(e);return (x >= eX + l && x <= eX + xWidth(e) - r &&y >= eY + t && y <= eY + xHeight(e) - b );
}
