...
##########################################################################
=head2 C<< pandora_generate_alerts (I<$pa_config> I<$data> I<$status> I<$agent> I<$module> I<$utimestamp> I<$dbh> I<$timestamp> I<$extraMacros> I<$last_data_value>) >>
...
sub pandora_generate_alerts ($$$$$$$$;$$) {
	my ($pa_config, $data, $status, $agent, $module, $utimestamp, $dbh, $timestamp, $extraMacros, $last_data_value) = @_;
	...
	foreach my $alert (@alerts) {
		...
		pandora_process_alert ($pa_config, $data, $agent, $module,
		                       $alert, $rc, $dbh, $timestamp, $extraMacros);
...
sub pandora_process_alert ($$$$$$$$;$) {
	my ($pa_config, $data, $agent, $module, $alert, $rc, $dbh, $timestamp, $extraMacros) = @_;
	...
	if ($rc == 4) {
		...
		pandora_execute_alert ($pa_config, $data, $agent, $module, $alert, 0, $dbh, $timestamp, $extraMacros);
		return;
	}
	...
	if ($rc == 0) {
		...
		pandora_execute_alert ($pa_config, $data, $agent, $module, $alert, 1, $dbh, $timestamp, $extraMacros);
		return;
	}
}
...
sub pandora_execute_alert ($$$$$$$$;$) {
	my ($pa_config, $data, $agent, $module,
	    $alert, $alert_mode, $dbh, $timestamp, $extraMacros) = @_;
	...
	foreach my $action (@actions) {
		pandora_execute_action ($pa_config, $data, $agent, $alert, $alert_mode, $action, $module, $dbh, $timestamp, $extraMacros);
	}
...
sub pandora_execute_action ($$$$$$$$$;$) {
	my ($pa_config, $data, $agent, $alert,
	    $alert_mode, $action, $module, $dbh, $timestamp, $extraMacros) = @_;
...
sub pandora_process_module ($$$$$$$$$;$) {
	...
	if (pandora_inhibit_alerts ($pa_config, $agent, $dbh, 0) == 0) {
		pandora_generate_alerts ($pa_config, $processed_data, $status, $agent, $module, $utimestamp, $dbh, $timestamp, $extraMacros, $last_data_value);
	} else {
